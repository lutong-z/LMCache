...
class Native432LMCacheMPConnector(LMCacheMPConnector):
    ...
    def __init__(self, vllm_config: Any, role: Any, kv_cache_config: Any = None):
        # LMCache must see EVERY engine group: vLLM marks connector-hit
        # tokens as computed for all groups, so any group LMCache cannot
        # load back becomes stale garbage (indexer/compressor/draft).
    ...
    def register_kv_caches(self, kv_caches: dict[str, Any]):
        ...
        edited_caches = apply_kv_cache_group_edits(
            full_config, kv_caches, layout_hints=layout_hints
        )
